...
"""
Clears and loads a DataFrame into MongoDB collection.
...
def load_csv_to_mongo(df: pd.DataFrame, collection, key_fields: list[str]):
    records = df.replace({pd.NA: None}).to_dict(orient="records") # Convert NaN to None for Mongo
...
def run_load():
    ...
    if not fills_path.exists() or not orders_path.exists(): raise FileNotFoundError("Expected ETL outputs not found under /output")
    # Load CSVs
    ...
    top_orders = list(db.orders.find({}, {"_id": 0}).sort("total_pi_dollars", -1).limit(5))
