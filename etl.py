...
def load_nbbo() -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: NBBO data sorted by symbol and timestamp.
    """
...
def load_orders() -> pd.DataFrame:
    ...
    Load orders.
    Returns:
        pd.DataFrame: Orders data with parsed timestamps.
    """
...
def process_trades_in_chunks(nbbo: pd.DataFrame, orders: pd.DataFrame) -> Iterator[pd.DataFrame]:
    """
    Process trades with metrics. Yield in successive chunks rather than
    loading full dataset at once because large data sets cannot fit into RAM.

    Args:
        nbbo (pd.DataFrame): NBBO quotes data sorted by symbol and timestamp.
        orders (pd.DataFrame): Orders data with order IDs and limit prices.

    Yields:
        pd.DataFrame: A processed chunk containing trades joined with NBBO
    ...
    for chunk in pd.read_csv(TRADES_PATH, parse_dates=["exec_timestamp"], chunksize=CHUNK_SIZE):
        ...
        # Align each trade with the latest NBBO before its timestamp
        aligned = pd.merge_asof(
            merged,
            nbbo,
            by="symbol",
            left_on="exec_timestamp",
            right_on="timestamp",
            direction="backward",
        )

        # Compute metrics
        aligned["pi_per_share"] = np.where(
            aligned["side_trade"] == "BUY",
            np.maximum(0, aligned["ask"] - aligned["exec_price"]),
            np.maximum(0, aligned["exec_price"] - aligned["bid"]),
        )
...
def run_etl() -> None:
    ...
    summary = (
        ...
        .agg(
            total_order_qty=("quantity", "first"),
            filled_qty=("exec_quantity", "sum"),
            vwap=(
                "exec_price",
                lambda x: np.average(x, weights=fills.loc[x.index, "exec_quantity"]),
            ),
            total_pi_dollars=("pi_dollars", "sum"),
        )
        .reset_index()
    )
...
def run_etl_from_paths(*, orders_path: Path, trades_path: Path, nbbo_path: Path) -> dict:
    ...
    Temporarily swaps the module-level paths, calls run_etl(), then restores them.
    Returns a small dict with output paths and the row counts written.
    """
    ...
    try:
        ...
        rows = {
            "fills": len(pd.read_csv(fills_path)) if fills_path.exists() else 0,
            "summary": len(pd.read_csv(summary_path)) if summary_path.exists() else 0,
        }
        ...
        ORDERS_PATH, TRADES_PATH, NBBO_PATH = old_orders, old_trades, old_nbbo
